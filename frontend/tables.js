...
const Councilors = (top6Councilors) => {1
    return table({class: "tb-councilor"},
        thead(
          tr(
            th("CANDIDATE"),
            th("VOTES"),
        ...
        tbody(
            top6Councilors.map(c => tr({style: `/*--background: ${shade(c.party_color, 0.731)};--width: 25%;*/--winnerPartyColor: ${c.party_color};`},
                td({title: c.total_votes},
                    c.full_name,
                ),
                td(
                    c.total_votes,
...
fetch('http://localhost:8055/total').then(async (response) => {
    ...
    van.add(document.getElementsByClassName('div3')[0], Councilors(d1Councilors))
    van.add(document.getElementsByClassName('div4')[0], Councilors(d2Councilors))
    van.add(document.getElementsByClassName('div5')[0], Councilors(d3Councilors))
})

function getTop6CouncilorPerDistrict(data, district, position) {
    const toSort = data;
    return toSort.filter(d => d.district === district && d.position === position)
...
function processData (data, position) {
    ...
    const [first, second, ...everyoneElse] = toProcess.filter(candidate => candidate.position === position)
        .sort((a, b) => a.total_votes > b.total_votes )
        .reverse()
